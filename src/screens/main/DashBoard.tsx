...
const DashBoard = () => {
  ...
  const { adminLocalData, adminDatabase, setAdminDatabase } = userContext();

  useEffect(() => {
    setloading(true);
    riciveData(adminLocalData.User_Phone_Number, setAdminDatabase).then(() => {
      setloading(false);
    });
  }, []);
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      {loading ? (
        ...
        <View style={styles.content}>
          ...
          {
            adminDatabase.User_Post_Count < 0 ? <>
              {/* Top Metrics Row */}
